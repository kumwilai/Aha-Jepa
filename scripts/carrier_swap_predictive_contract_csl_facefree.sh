#!/usr/bin/env bash
# CSL-Daily predictive-contract carrier swap with the carrier face block frozen.
# This tests whether removing HRNet face-coordinate variance restores the CSL
# latent precondition: JEPA should become the richest and most organised carrier
# over the signing articulators, then separate under MSKA gloss-WER.
#
#   usage: scripts/carrier_swap_predictive_contract_csl_facefree.sh <SEED> [jepa mae ae]
set -uo pipefail
...
CSW=$ROOT/carrier_swap_csl_facefree
...
run() {
  OBJ="$1"
  TAG=${OBJ}_pc_ff_s${SEED}
  LOG=$CSW/${TAG}.log
  banner(){ echo; echo "==== [$(date +%H:%M:%S)] csl-pc-facefree seed=$SEED OBJ=$OBJ $* ===="; }
  fail(){ echo "csl-pc-facefree seed=$SEED OBJ=$OBJ FAILED at: $*"; return 1; }
  {
  CARRIER=$CSW/carrier_${OBJ}_ff_s${SEED}
  banner "1. carrier (--objective $OBJ, face frozen, 12 ep)"
  if [ -f "$CARRIER/best.pt" ]; then echo "skip carrier"; else
    $PY -u scripts/aha_jepa_csl.py jepa \
      ...
      --seed "$SEED" --epochs 12 || { fail carrier; return 1; }
  fi
  [ -f "$CARRIER/best.pt" ] || { fail "no carrier best.pt"; return 1; }

  V25=$CSW/${OBJ}_v25pc_ff_s${SEED}
...
echo; echo "==== [$(date +%H:%M:%S)] MSKA-SLR gloss-WER scoring facefree (seed $SEED) ===="
SRC_TAGS=(); for OBJ in "${OBJS[@]}"; do SRC_TAGS+=("${OBJ}_pc_ff_s${SEED}"); done
...
echo "==== carrier_swap_predictive_contract_csl_facefree seed $SEED complete ===="
