#!/usr/bin/env bash
# CSL-Daily predictive-contract swap for the proposed "rich articulator JEPA" fix.
# All carriers use the same face-frozen carrier view; JEPA additionally receives
# a rank-spread and lexical-organisation auxiliary during carrier pretraining.
# This is the direct test of the CSL rich-latent failure mode.
#
#   usage: scripts/carrier_swap_predictive_contract_csl_articjepa.sh <SEED> [jepa mae ae]
set -uo pipefail
...
CSW=$ROOT/carrier_swap_csl_articjepa
...
run() {
  OBJ="$1"
  TAG=${OBJ}_pc_artic_s${SEED}
  LOG=$CSW/${TAG}.log
  banner(){ echo; echo "==== [$(date +%H:%M:%S)] csl-pc-artic seed=$SEED OBJ=$OBJ $* ===="; }
  fail(){ echo "csl-pc-artic seed=$SEED OBJ=$OBJ FAILED at: $*"; return 1; }
  {
  CARRIER=$CSW/carrier_${OBJ}_artic_s${SEED}
  EXTRA=()
  if [ "$OBJ" = "jepa" ]; then
    EXTRA=(--lambda_latent_rank 0.05 --lambda_lex_ce 0.01)
  fi
  banner "1. carrier (--objective $OBJ, face frozen, rich-JEPA aux if applicable, 12 ep)"
  if [ -f "$CARRIER/best.pt" ]; then echo "skip carrier"; else
    $PY -u scripts/aha_jepa_csl.py jepa \
      --out_dir "$CARRIER" --objective "$OBJ" --carrier_face_weight 0.0 \
      "${EXTRA[@]}" --seed "$SEED" --epochs 12 || { fail carrier; return 1; }
  fi
  [ -f "$CARRIER/best.pt" ] || { fail "no carrier best.pt"; return 1; }

  V25=$CSW/${OBJ}_v25pc_artic_s${SEED}
  ...
  V29=$CSW/${OBJ}_v29pc_artic_s${SEED}
...
echo; echo "==== [$(date +%H:%M:%S)] MSKA-SLR gloss-WER scoring artic-JEPA (seed $SEED) ===="
SRC_TAGS=(); for OBJ in "${OBJS[@]}"; do SRC_TAGS+=("${OBJ}_pc_artic_s${SEED}"); done
...
echo "==== carrier_swap_predictive_contract_csl_articjepa seed $SEED complete ===="
